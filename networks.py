...
class Generator(nn.Module):
    def __init__(self,input_dim,parse_dim,ngf):
        super(Generator, self).__init__()
        self.ngf=ngf
        self.content_style_separation = Content_Style_Separation()
    ...
    def forward(self,source_img,source_parse,source_all_mask,ref_img,ref_parse,ref_all_mask,replace_content_img=None):
        ...
        if replace_content_img is not None:
            source_face = replace_content_img * source_all_mask
            source_face_content, source_face_style = self.content_style_separation(source_face)
        ...
        ref_makeup=F.interpolate(ref_face_style,scale_factor=0.25)
...
class Content_Style_Separation(nn.Module):
    ...
    def forward(self,img):
        ...
        # Increase randomness to prevent the network from inferring approximate colors based on grayscale
        content=content*(0.7+0.6*random.random())
        # b = content.shape[0]
        # rand_b = torch.rand(size=(b, 1, 1, 1)).to(content.device)
        # rand_b = rand_b * 0.4 - 0.2
        # content = content + rand_b
